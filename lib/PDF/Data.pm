...
sub parse_pdf {
  ...
  # Parse PDF objects.
  $self->parse_objects(\$data, $offset);

  # Check for startxref value.
  my $startxref = $self->{-startxref};
  ...
  while ($startxref) {
    ...
    my $trailer = (sort { $a->[0] <=> $b->[0] } map [abs($_->{-offset} - $startxref), $_], @{$trailers})[0][1];
...
sub minify_content_stream {
  ...
  die "Content stream serialization failed"
    if dump($objects) ne dump([$self->parse_objects(\($stream->{-data} //= ""), 0)]);
}
...
sub generate_content_stream {
  ...
  foreach my $object (@{$objects}) {
    ...
    if (ref $object eq 'HASH' and $object->{-image}) {
      # Serialize inline image data.
      $self->serialize_image(\$stream, $object->{-image});
    } elsif (ref $object eq 'HASH') {
      # Serialize dictionary.
      $self->serialize_dictionary(\$stream, $object);
    } elsif (ref $object eq 'ARRAY') {
      # Serialize array.
      $self->serialize_array(\$stream, $object);
    } else {
      # Serialize string or other token.
      $self->serialize_object(\$stream, $object);
    }
  }
...
#
# Static (0-5) — same in every dispatch array:
#  0 = parse error
#  1 = whitespace (no-op)
#  2 = obj
#  3 = startxref
...
#  $4  = generation number
#  $5  = number value
#  $6  = clean string (no parens/backslash/CR/LF)
...
sub parse_objects {
  ...
  *_ = ref($data) eq "SCALAR" ? $data : \$data;
  ...
  my $self = $me;
  my ($key, $dict, $array, $token_start, $token_end);
  ...
  # ===================================================================
  my ($key_mode, $val_mode, $arr_mode, $obj_mode, $top_dispatch);
  ...
  # Trailer: one-shot hook on store_dict (slot 17).
  # Creates one closure per trailer keyword (1-2 per file, negligible).
  ...
  # ===================================================================
  # Dict key-mode closures (index 6-22).
  # Only name tokens are valid; everything else is an error.
  # ===================================================================
  ...
  # ===================================================================
  # Dict val-mode closures (index 6-22).
  # ===================================================================
  my $val_name  = sub { $dict->{$key} = $1;  $dispatch = $key_mode };
  my $val_num   = sub { $dict->{$key} = $5;  $dispatch = $key_mode };
  my $val_cstr  = sub { $dict->{$key} = $6;  $dispatch = $key_mode };
  ...
  # ===================================================================
  # Array-mode closures (index 6-22).
  # ===================================================================
  my $arr_name  = sub { push @$array, $1 };
  my $arr_num   = sub { push @$array, $5 };
  my $arr_cstr  = sub { push @$array, $6 };
  ...
  # ===================================================================
  # Obj-mode closures (index 6-22).
  # Every store registers the indirect object immediately.
  # ===================================================================
  my $obj_name = sub { $obj_value = $1; &$obj_register };
  my $obj_num  = sub { $obj_value = $5; &$obj_register };
  my $obj_cstr = sub { $obj_value = $6; &$obj_register };
  my $obj_bool = sub { $obj_value = $12; &$obj_register };
  my $obj_null = sub { $obj_value = "null"; &$obj_register };
  my $obj_tok  = sub { $obj_value = $13; &$obj_register };
  ...
  # ===================================================================
  # Top-level closures (index 6-22).
  # ===================================================================
  my $top_name  = sub { push @objects, $1 };
  my $top_num   = sub { push @objects, $5 };
  my $top_cstr  = sub { push @objects, $6 };
  ...
  # ===================================================================
  # Build dispatch arrays: [@static, dynamic closures 6..22].
  # ===================================================================
  $key_mode = [@static,
    $key_plain_name,     #  6
    $key_hex_name,       #  7
    ...
    $key_error,          # 18: [
    $close_dict,         # 19: >>
    $error_close_array,  # 20: ]
    $error_endobj,       # 21: endobj
    $error_stream,       # 22: stream
  ];
  ...
  $val_mode = [@static,
    ...
    $val_store_dict,     # 17
    $val_store_array,    # 18
    $key_missing_value,  # 19: >> (missing value)
    $error_close_array,  # 20
    $error_endobj,       # 21
    $error_stream,       # 22
  ];
...
# Parse compressed object stream into individual indirect objects.
sub parse_object_stream {
  my ($self, $stream) = @_;

  # Validate extends chain (skip if extends an unresolved or non-stream object).
  # NOTE: Preserve existing $extends validation logic from original; shown here
  # only as a structural placeholder.  The key change is positional registration
  # below instead of metadata-based iteration.
  my $extends = $stream->{Extends};
  if (defined $extends) {
    $extends = $self->{-indirect_objects}{$extends}{data}
      if defined $extends and !ref $extends and $self->{-indirect_objects}{$extends};
    return if !defined $extends or (ref $extends and reftype($extends) eq "SCALAR");
    return unless is_stream($extends);
  }

  my $data = $stream->{-data};
  my $n    = $stream->{N} or return;

  # Extract object numbers from header (N pairs of object_number + byte_offset).
  my @ids;
  pos($data) = 0;
  for (1 .. $n) {
    $data =~ /\G\s*(\d+)\s+\d+/gc
      or croak join(": ", $self->file || (), "Invalid object stream header!\n");
    push @ids, $1;
  }

  # Parse remaining content — returns raw values positionally.
  my @values = $self->parse_objects(\$data, pos($data));

  # Register objects by positional correspondence with header IDs.
  my @stream_objects;
  for my $i (0 .. $#ids) {
    my $id     = $ids[$i];
    my $value  = $values[$i];
    my $object = { data => $value, id => $id };
  ...
  # Store on stream dict for Extends chain processing in parse_pdf.
...
C<$pdf-E<gt>validate_content_stream()> and C<$pdf-E<gt>minify_content_stream()>,
and called by itself recursively, to parse PDF objects into Perl representations.
